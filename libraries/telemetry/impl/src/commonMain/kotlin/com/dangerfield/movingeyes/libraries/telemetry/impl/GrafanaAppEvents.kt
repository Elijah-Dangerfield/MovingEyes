...
import com.dangerfield.movingeyes.libraries.core.TelemetryInfo
    ...
    init {
        if (GrafanaCloud.isConfigured) {
            val tree = GrafanaLogTree(
                ...
                klogForwardingEnabled = { klogForwardingEnabled() },
                currentSessionId = { sessionTracker.current.uuid },
...
/**
 * Grafana Cloud OTLP gateway credentials, injected at build time (see
 * `loadTelemetryMetadata` in build-logic): CI reads GitHub secrets, local
 * builds read `local.properties` `grafana.*` keys. The repo is public, so the
 * token never lives in source — even though it is write-only and ships in the
 * binary regardless (any client credential is extractable; this one can only
 * append logs).
 *
...
internal object GrafanaCloud {
    val OTLP_BASE_URL: String = TelemetryInfo.grafanaOtlpBaseUrl
    val INSTANCE_ID: String = TelemetryInfo.grafanaOtlpInstanceId
    val LOGS_WRITE_TOKEN: String = TelemetryInfo.grafanaLogsWriteToken
