...
Package root: **`com.dangerfield.movingeyes.server`** (keep dir == package; do **not**
...
apps/server/
  ...
  src/main/kotlin/com/movingeyes/server/
  ...
  src/main/resources/
    ...
    logback.xml
  src/test/kotlin/com/movingeyes/server/
...
- **build-logic**: make the KMP convention plugins skip Android/iOS targets when
  `-Dmovingeyes.serverOnly=true` (mirrors Cards) so a server Docker build needs
  no mobile toolchain.
