...
- **`versionCode` / iOS build number** — auto-overridden in CI with `GITHUB_RUN_NUMBER` via env vars `VERSION_CODE_OVERRIDE` and `BUILD_NUMBER_OVERRIDE`. They bump monotonically without commits. (See [Versioning.kt](../build-logic/src/main/java/com/movingeyes/util/Versioning.kt).)
- **Sentry release ID**: `movingeyes@{version}+{build}` (e.g. `movingeyes@0.2.0+42`). Created on both platforms in release.yml.
...
| Name | Kind | Value |
|---|---|---|
| `ANDROID_KEYSTORE_BASE64` | secret | `base64 -i movingeyes-upload.p12 \| pbcopy` |
| `ANDROID_KEYSTORE_PASSWORD` | secret | Set when keystore was created. |
| `ANDROID_KEY_ALIAS` | secret | e.g. `movingeyes-release`. |
...
```sh
keytool -genkey -v -keystore movingeyes-upload.p12 -storetype PKCS12 \
  -keyalg RSA -keysize 4096 -validity 10000 -alias movingeyes-release
```
...
3. IAM & Admin → Service Accounts → Create → name `movingeyes-ci`, role none.
...
- [ ] Create the app entry for `com.dangerfield.movingeyes`
