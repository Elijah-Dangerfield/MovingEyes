...
fun main() {
    printBlue("🚀 Moving Eyes Module Creator")
    println()
...
fun checkForHelp(): Boolean {
    ...
    if (isHelpRequest) {
        printBlue("""
            Moving Eyes Module Creator
            
            This script creates new KMP modules for the Moving Eyes project with proper structure and configuration.
...
fun createSourceStructure(config: ModuleConfig, moduleDir: File) {
    val packagePath = "com.dangerfield.movingeyes.${config.baseDir}.${config.fullName.replace(":", ".")}".replace(".", "/")
...
fun createMainSourceFile(config: ModuleConfig, kotlinDir: File) {
    val packageName = "com.dangerfield.movingeyes.${config.baseDir}.${config.fullName.replace(":", ".")}"
...
fun createFeatureEntryPoint(config: ModuleConfig, kotlinDir: File, packageName: String) {
    ...
    val (routePackage, routeClass) = if (config.moduleName == "impl" && config.parentModule != null) {
        ...
        val parentPackage = "com.dangerfield.movingeyes.${config.baseDir}.${config.parentModule}"
...
${routeImport}import com.dangerfield.movingeyes.libraries.navigation.FeatureEntryPoint
...
fun createBuildGradle(config: ModuleConfig, moduleDir: File) {
    ...
    val pluginId = when (config.type) {
        ModuleType.FEATURE -> "movingeyes.feature"
        ModuleType.LIBRARY -> "movingeyes.kotlin.multiplatform"
    }
    
    val namespace = "com.dangerfield.movingeyes.${config.baseDir}.${config.fullName.replace(":", ".")}"
